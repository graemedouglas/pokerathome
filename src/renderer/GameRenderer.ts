...
export class GameRenderer {
  ...
  async init(): Promise<void> {
    ...
    // Phase indicator — white with dark stroke for visibility over green felt
    ...
    // Title — with more spacing from gear button
    ...
    // Gear button (settings) — proper gear/cog icon
  ...
  update(state: GameState): void {
    ...
    if (state.phase === 'showdown' && state.winners.length > 0) {
      const lines = state.winners.map(w => {
        ...
        return `${player.name} wins $${w.amount.toLocaleString()} — ${w.handDescription}`;
      });
