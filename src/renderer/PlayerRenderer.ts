...
const BET_OFFSETS: { x: number; y: number }[] = [
  { x: 70, y: -35 },     // seat 0 (bottom) → upper-right
  { x: 50, y: -30 },     // seat 1 (lower-left) → upper-right
  { x: 50, y: 30 },      // seat 2 (upper-left) → lower-right
  { x: -70, y: 35 },     // seat 3 (top) → lower-left
  { x: -50, y: 30 },     // seat 4 (upper-right) → lower-left
  { x: -50, y: -30 },    // seat 5 (lower-right) → upper-left
];
...
export class PlayerRenderer extends Container {
  ...
  update(player: Player, phase: GamePhase, isWinner: boolean): void {
    // Avatar — positioned to the side of the panel
